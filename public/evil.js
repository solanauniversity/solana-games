...
(function () {
  function addScore(j) {
    if (!document.getElementById("score").innerHTML)
      document.getElementById("score").innerText = j;
    document.getElementById("score").className = "end";
  }
  function rmScore() {
    if (document.getElementById("score").innerHTML != "") {
  ...
  function C(a, b, e, h, f, k) {
    ...
    l = l || f.length;
    k = k || 0;
    for (var n = 25 > h ? 10 : 0.5 * h, p = 0; p < l; p++)
      Ob[k](a, b + p, e + p, h, f[p] || f[0], n);
  }
  function gb(a) {
  ...
  function Ta(a) {
    for (var b = {}, e, h = 0; 9 > h; h++)
      if (
        ((e = gb([a[0] + 84 * ((h % 3) - 1), a[1] + 84 * (~~(h / 3) - 1)])),
        !b[e])
      )
  ...
  function jb() {
    ...
    for (a = 0; a < Z - 1; a++)
      10 != a &&
        ((ra[45e4 + 6e3 * a] = [0, a, a, 10]),
        (ra[453e3 + 6e3 * a] = [0, Z - a - 1, a, 10]));
  ...
  function qb(a, b) {
    var e = ua([
      a[0] + 10 * Math.cos((Math.PI * b) / 3),
      a[1] + 10 * Math.sin((Math.PI * b) / 3),
      4,
    ]);
    e[13] = a;
    e[9] = Ha(e, a);
  ...
  function ua(a) {
    ...
    if (12 == a[5] || 14 == a[5]) for (var b = 0; 6 > b; b++) qb(a, b);
    return a;
  }
  function Qb(a) {
    if (0.99 < a) return 1;
  ...
  function rb(a) {
    ...
    if (Ca) Da++, 30 < Da && (Da = 0);
      ...
      1694 < ka &&
        ...
        (U = []),
        (za = !0),
        Ja.setItem("agar3sjs13k-gm", "qyui"),
        mb());
      ka++;
    } else if (0 > E[6]) {
      a = dt;
      if (!ea) {
        ...
        rmScore();
        T[Math.round(f[1] / n)] &&
          1 == T[Math.round(f[1] / n)][Math.round(f[0] / n)] &&
          (b -= 0.5);
        ca & da[65] &&
          ((f[0] -= b),
          f[0] < f[2] && (f[0] = f[2]),
          f[0] > l[2] && f[0] < v - l[2] && (l[0] += b),
          32 < l[0] && (l[0] = 32));
        ca & da[87] &&
          ((f[1] -= b),
          f[1] < f[2] && (f[1] = f[2]),
          f[1] > l[3] && f[1] < v - l[3] && (l[1] += b),
          27 < l[1] && (l[1] = 27));
        ca & da[83] &&
          ((f[1] += b),
          f[1] > v - f[2] && (f[1] = v - f[2]),
        ...
        if (Ta(f)) {
          ...
          if ((Ka = J > record))
            (record = J),
              Ja.setItem("agar3sjs13k-record", J),
              (W[1][3] = !0),
              (W[2][3] = !0);
          ja = 0;
        }
        B[2] && 0 >= f[6] && 0 >= f[8]
          ? (U.push([
              ...
              f[1] + Ba(1, 2 + f[7] / 30),
              2,
              f[3] + Ba(1, 0.05 + 0.001 * f[7]),
            ]),
            O(sb[~~w(sb.length)]),
            (f[6] = 1 / f[7]))
          : (f[6] -= a);
        B[3] && 0 >= f[8] && 0 >= f[9]
          ? (O(tb), (f[8] = 0.55), (f[9] = 1.2))
          : ((f[8] -= a), (f[9] -= a));
      }
      a = U.length - 1;
      for (; 0 <= a; a--) {
        ...
        (-20 > b[0] || b[0] > v + 20 || -20 > b[1] || b[1] > v + 20) &&
          U.splice(a, 1);
        var q = Ta(b);
        q &&
          (0 < --q[6] && $a(b[0], b[1], -b[3], 2, 10, 9),
          U.splice(a, 1),
          (q[4] = 200),
          9 < q[5] && O(ub[~~w(ub.length)]));
      }
      for (a = 0; a < oa.length; a++)
        (b = oa[a]),
          (b[0] += Math.cos(b[2]) * w(3, 2)),
          (b[1] += Math.sin(b[2]) * w(3, 2)),
          0 > --b[3] && oa.splice(a, 1);
      0 < ba && (ba -= 0.1);
      ia = {};
      for (a = F.length - 1; 0 <= a; a--)
        a: if (((b = F[a]), (q = a), 0 >= b[6])) {
          if ((F.splice(q, 1), 5 != b[5]))
            if (
              (14 == b[5] && (Ea = !0),
              $a(b[0], b[1], b[2], Ma[b[5]][0], 2 * Ma[b[5]][0], b[5] + 24),
              9 < b[5])
            ) {
              for (
                var q = b[0],
          ...
          if (10 > b[5]) {
            ...
            4 != b[5]
              ? ((b[0] += Math.cos(b[9]) * t * b[12]),
                (b[1] += Math.sin(b[9]) * t * b[12]))
              : (1 > b[13][6] && (b[10] *= 0.99),
                (b[15] = 2 * n * (-Math.cos(b[16]) + 1.2)),
                (b[16] += t / 200),
                (b[0] = b[13][0] + Math.cos(b[9]) * b[15]),
                (b[1] = b[13][1] + Math.sin(b[9]) * b[15]));
          } else {
            b[9] -= t;
            12 <= b[5] && (b[3] += b[12] * t);
            ...
            for (Y = X - G; Y < X + G; Y++)
              if ("undefined" != typeof T[Y])
                for (var u = x - G; u < x + G; u++)
                  1 == T[Y][u] ||
                    Ra((Y + 0.5) * n - h, (u + 0.5) * n - q) >= m ||
                    (T[Y][u] = 1);
          }
          q = gb(b);
          ia[q] = ia[q] || [];
      ...
      if (0 < V.length && J > V[0])
        switch (((a = ra[V.splice(0, 1)[0]]), a.splice(0, 1)[0])) {
          case 0:
            a[0] = (a[0] + 0.5) * n;
            a[1] = (a[1] + 0.5) * n;
            La(a[0], a[1], 1, a);
    ...
    if (Ca) {
      ...
      e.beginPath();
      for (a = 0; 10 > a; a++)
        (b = cb(30 * a + Da)), p(1), M(0, b + 0.5, 800, void 0, 0);
      e.stroke();
      e.beginPath();
      for (a = 0; 10 > a; a++)
        (b = cb(30 * a + Da)), p(2), M(0, 600 - b - 0.5, 800, void 0, 0);
      e.stroke();
      p(2);
      e.beginPath();
      ...
      for (a = 1; a < 800 / 60; a++)
        (b = a * a * 5 + 25),
          p(1),
          M(30 * a + 400, 360, 240, !0, b),
          M(30 * -a + 400, 360, 240, !0, -b);
      e.stroke();
      va
        ? (C("controls", 400, 130, 12, [0, 16]),
          C("move             awsd", 400, 251, 12, [0, 0]),
          C("fire       left click", 400, 290, 12, [0, 0]),
          C("warptime  right click", 400, 330, 12, [0, 0]))
        : (C("winners don't use drugs", 401, 50, 9, [0, 0]),
          C("evil glitch", 400, 270 - 50 * N, 30 * (1 + N), [0, 9, 0, 9]));
      ...
      D = ea
        ? [0, 0]
        : [Ba(B[2] || 0 < ba, ba + 2), Ba(B[2] || 0 < ba, ba + 2)];
      p(-1, 1, "rgba(7,8,12," + (0.2 - (0 < f[8] ? 0.1 : 0)) + ")");
      e.translate(l[0] + D[0], l[1] + D[1]);
      e.fillRect(0, 0, v, v);
      p(1);
      e.beginPath();
      for (a = 0; a <= Z; a++) Xa(a * n - 0.5, v);
      e.stroke();
      e.beginPath();
      p(5);
      for (a = 0; a <= Z; a++) Xa(a * n + 0.5, v);
      ...
      for (b = 0; b < Z; b++)
        for (a = 0; a < Z; a++)
          0 != T[b][a] &&
            (e.fillRect(a * n + l[0] + D[0], b * n + l[1] + D[1], n, n),
            e.strokeRect(
              a * n + l[0] + D[0] - 0.5,
              b * n + l[1] + D[1] - 0.5,
              n + 2,
        ...
        (m = ta[a]),
          ...
          14 == h
            ? ((h = x),
              e.translate(h, m),
              e.beginPath(),
              p(-1, 1, "rgba(210,0,0,0.9)"),
              e.arc(0, 0, v * (1 - q), 0, 2 * Math.PI),
              e.stroke(),
              e.fill(),
              e.beginPath(),
              p(0),
              0.3 > q
                ? (e.moveTo((-b * q) / 0.3, 0), e.lineTo((b * q) / 0.3, 0))
                : (p(0, 1),
                  e.bezierCurveTo(-b, 0, 0, (-b * q) / 3.5, b, 0),
                  e.bezierCurveTo(b, 0, 0, (b * q) / 3.5, -b, 0),
                  e.fill()),
              e.closePath(),
              e.translate(-h, -m))
            : (p(-1, 0, "rgba(38,82,255," + q + ")"),
              e.fillRect(x - q * b, m - q * b, q * b * 2, q * b * 2));
      e.closePath();
      e.fill();
      e.stroke();
      e.restore();
      ...
      for (a = 0; a < F.length; a++)
        if (
          ...
          b = h[0] + l[0] + D[0] + ((0.5 < w() ? 1 : -1) * h[4]) / 40;
          q =
            h[1] +
            l[1] +
            D[1] +
            ((0.5 < w() ? 1 : -1) * h[4]) / 40 -
            (9 < h[5] ? 5 * Math.sin((ja / 50) % (2 * Math.PI)) + 5 : 0);
          e.translate(b, q);
          e.beginPath();
          if (10 > h[5])
            p(h[5] + 24),
            ...
            for (x = 0; 6 > x; x++)
              (X = ga[2 * x]),
                (G = ga[2 * x + 1]),
                e.beginPath(),
                e.arc(4 * X * m, 4 * G * m, m, 0, 2 * Math.PI, !1),
                e.stroke(),
                e.beginPath(),
                e.arc(2 * X * m, 2 * G * m, m, 0, 2 * Math.PI, !1),
                e.stroke(),
                e.beginPath(),
                e.moveTo(X * m * 4, G * m * 4),
                e.lineTo(
                  4 * m * ga[(2 * x + 2) % 12],
                  4 * m * ga[(2 * x + 3) % 12]
                ),
            ...
            for (
              p(-1, 2, 2),
                m = (Fa[h[5]] - h[9]) / Fa[h[5]],
                m = 0 < h[4] ? -55 : ~~(200 * m) * wb[m.toFixed(4)],
                p(16),
                x = 0;
              x < h[7].length;
              x++
      ...
      for (a = 0; a < U.length; a++)
        (b = U[a]),
          20 > b[0] + l[0] ||
            ...
            b[1] + l[1] > H - 20 ||
            (e.beginPath(),
            e.arc(b[0] + l[0], b[1] + l[1], b[2], 0, 2 * Math.PI, !1),
            e.closePath(),
            e.fill());
      e.restore();
      e.save();
      ...
      ea
        ? (p(22, 1),
          e.fillRect(0, 0, v, v),
          za && C("evil mode", 400, 80, 22, [0, 16]),
          Ka
            ? (C("-new record-", 400, 240, 22, [10, 18]),
              C("-share it-", 400, 400, 14, [24, 18]))
            : C("game over", 400, 240, 20, [0, 13]),
          addScore(J.toFixed(0)),
          C(J.toFixed(0), 400, 160, Ka ? 20 : 16, [0, 9]))
        : (C(6 < wave ? "evil" : wave + "/6", 400, 60, 9, [0, 3]),
          C(J.toFixed(0), 750, 60, 18, [32, 9], 1),
          C(
            J > record ? "record" : record.toFixed(0),
            750,
            110,
            9,
            [24, 3],
    ...
    for (a = 0; a < W.length; a++)
      (b = W[a]),
        b[3] &&
          ...
          p(q),
          p(-1, 2, 2),
          e.strokeRect(b[0], b[1], b[2], 42),
          C(b[5], b[0] + b[2] / 2, b[1] + 9, 12, [0, q]));
    e.restore();
    Oa
      ? (e.save(),
        e.beginPath(),
      ...
      : !Ea ||
        ...
        e.beginPath(),
        436 > ka
          ? (p(-1, 1, "rgba(0,0,0," + (1 - (436 - ka) / 120) + ")"),
            e.fillRect(0, 0, 800, 600))
          : (p(23, 1), e.fillRect(0, 0, 800, 600), C(sa, 400, 250, 16, a)),
        e.closePath(),
        e.fill(),
        e.restore());
    0 < N && ((N += 0.05), 0.51 == N && O(Yb));
    1 < N && ((Ca = !1), (N = 0), jb());
    0 < N &&
      1 > N &&
      (p(-1, 1, "rgba(220,220,220," + N + ")"), e.fillRect(0, 0, 800, 600));
    (Oa = 0 < Ya) && --Ya;
    e.restore();
    a = g;
  ...
  var Sa = new (function () {
      ...
      this.r = function () {
        ...
        h = 100 / (a.f * a.f + 0.001);
        f = 100 / (a.g * a.g + 0.001);
        k = 1 - a.h * a.h * a.h * 0.01;
        l = -a.i * a.i * a.i * 1e-6;
        a.a || ((v = 0.5 - a.n / 2), (B = 5e-5 * -a.o));
        n = 1 + a.l * a.l * (0 < a.l ? -0.9 : 10);
        p = 0;
      ...
      this.U = function (C, D) {
        ...
        for (qa = 0; qa < D; qa++) {
          ...
          H &&
            ((K += T), (Q = K | 0), 0 > Q ? (Q = -Q) : 1023 < Q && (Q = 1023));
          E && F && ((z *= F), 1e-5 > z ? (z = 1e-5) : 0.1 < z && (z = 0.1));
          ca = 0;
          for (var ra = 8; ra--; ) {
            P++;
            if (P >= na && ((P %= na), 3 == M))
            ...
            switch (M) {
              ...
              case 2:
                y = P / na;
                y = 6.28318531 * (0.5 < y ? y - 1 : y);
                y = 1.27323954 * y + 0.405284735 * y * y * (0 > y ? 1 : -1);
                y = 0.225 * ((0 > y ? -1 : 1) * y * y - y) + y;
                break;
              case 3:
  ...
  if (Ia) {
    ...
    bb.gain.value = window.chrome ? 0.2 : 0.4;
    Pa.connect(bb);
  ...
  la.M = function (a) {
    ...
    return (
      Zb *
      Math.pow(Math.pow(2, 1 / 12), Fb[a[0]]) *
      Math.pow(2, (a[1] || 4) - 4)
    );
  };
  la.L = function (a) {
  ...
  A.prototype.createOscillator = function () {
    ...
    this.K
      ? this.A.setPeriodicWave(this.C.createPeriodicWave.apply(this.C, this.K))
  ...
  c.onmousemove = function (a) {
    0 < E[6] ||
      ((B[0] = (800 * a.offsetX) / c.offsetWidth),
      (B[1] = (600 * a.offsetY) / c.offsetHeight));
  };
  var ca = 0,
    da = { 65: 1, 87: 2, 68: 4, 83: 8 };
  ...
  z.I = 0.35;
  K.F = 0.05;
  fa.gain.gain.value = 0.12;
  L.gain.gain.value = 0.09;
  z.gain.gain.value = 0.11;
  K.gain.gain.value = 0.1;
  var Yb = S([
      3,
      0.2421,
      0.1876,
  ...
  for (
    var W = [
      ...
      ta = [],
      vb = [10, 10, 11, 11, 11, 12, 12, 10, 10, 11, 13, 10, 11, 12, 12],
      ab = 0,
      Ma = [
        [
          15,
      ...
      Fa = { 10: 2800, 11: 2600, 12: 60, 13: 200 },
      ba = 0,
      ga = [],
      P = 0;
    6 > P;
  ...
  var Vb = {
      ...
      14: function (a) {
        ...
        0 == a[13] % 16 &&
          ((e = ua([a[0], a[1], a[15][a[14] % a[15].length]])),
          (e[9] = a[3] / 2),
          F.push(e),
          a[14]++);
        if (0 == a[13] % 100) for (b = 0; 6 > b; b++) qb(a, b);
        a[9] = 70;
        a[13]++;
      },
    },
